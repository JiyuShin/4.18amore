...
export function deleteBrowserProjects(projectIds = []) {
  const ids = Array.isArray(projectIds)
    ? [...new Set(projectIds.filter((projectId) => typeof projectId === "string" && projectId.trim()))]
    : [];
  if (!ids.length) return listBrowserProjects();
